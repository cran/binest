...
###########################################################################################
mle_hetop <- function(ngk, iterlim = 1500, ...){
    ...
    ## Derive the two fixed cutpoints internally on the standardized scale,
    ## from the pooled bin proportions.  The starting values for mu and
    ## log(sigma) (computed below) are always on a standardized scale
    ## (mean 0, SD 1).  Earlier versions of this function allowed the user
    ## to supply cutpoints via a `fixedcuts` argument; when those cutpoints
    ## were on a native test-score scale (e.g. STAAR scale scores), the
    ## log likelihood was numerically flat near the starting values, nlm()
    ## could not make progress, and the function silently returned the
    ## starting values disguised as MLE estimates.  Deriving the cutpoints
    ## internally ensures the cutpoints, starting means, and starting
    ## log-SDs all live on the same scale.
    ...
    negll <- function(param){
        ...
        ## Calculate cell probabilities (vectorized).
        ## For each cutpoint c, compute pnorm(c, mean=.mug, sd=.sigmag)
        ## across all groups in one call; the result is a G x (K-1) matrix
        ## of cumulative bin probabilities, which we differenced to get
        ## bin probabilities and bordered with 0 on the left and 1 on the right.
        Phi <- sapply(.cutpoints, function(c) pnorm(c, mean=.mug, sd=.sigmag))
        if(G == 1) Phi <- matrix(Phi, nrow=1)
        pgk <- cbind(Phi, 1) - cbind(0, Phi)
    ...
    ## Compute starting values internally on the standardized scale.
    ## NOTE: sd(mu) is NA if G=1 so we then just set mu = 0.
    cats <- 1:K
    ...
    if(res$iterations == 0){
        warning("nlm reported zero iterations; estimates equal starting values and are not the MLE. This is unexpected with internally derived cutpoints and may indicate a problem with the bin counts.")
    }
