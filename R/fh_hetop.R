fh_hetop <- function(ngk, fixedcuts, p, m, gridL, gridU, Xm=NULL, Xs=NULL, seed=12345, modelfileonly = FALSE, modloc=NULL, ...){
    ...
    cat("model
  {
    ...
    if (!requireNamespace("R2jags", quietly = TRUE)) {
        stop("fh_hetop() requires the R2jags package (and an external ",
             "JAGS installation from https://sourceforge.net/projects/mcmc-jags/). ",
             "Install R2jags with install.packages(\"R2jags\") after installing JAGS.")
    }
    ## Suppress JAGS' "Compiling model graph / Resolving undeclared
    ## variables / Allocating nodes" stdout messages, which JAGS prints
    ## from its C++ library and which R cannot otherwise control.
    ## Users who want to see the JAGS startup chatter can recover it
    ## with `print(r)` on the returned object's `BUGSoutput`.
